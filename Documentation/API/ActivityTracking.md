...
`MotionObservation` is Equatable and holds all six possibly-overlapping classification flags (`walking`, `running`, `cycling`, `automotive`, `stationary`, `unknown`), confidence, and native start date. The `unknown` initializer argument defaults to false for source compatibility. Explicit unknown is preserved separately from an observation where all flags are false; both resolve to `.unknown` in the single-state projection.

`latestObservation` exposes the complete latest snapshot, or nil before the first update. `onObservation` receives every snapshot after coordinator state updates, including repeats, low-confidence updates, and unknown classifications. This callback is separate from `onChange`, so a host can subscribe without replacing the tracking engine's motion handler. For example, `automotive && stationary` can represent a stopped vehicle; it is not by itself proof of parking or a dwell.

`confirmedState` is the last known medium/high-confidence state, or nil before one arrives. It remains unchanged on low-confidence or unknown updates and is not a guarantee of current activity. Existing `state`, precedence, and filtered `onChange` semantics are unchanged.
